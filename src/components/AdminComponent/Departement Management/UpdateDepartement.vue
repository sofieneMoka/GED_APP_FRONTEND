...
<form @submit.prevent="UpdateDepartement">
  <div class="form-field">
    <input type="text" placeholder="Name" v-model="name" required/>
  </div>

    <div class="form-field">
    <select name="nameDirection" id="role-select" class="departmentoption" v-model="nameDirection" required>
        <option 
...
form .form-field:nth-child(2)::before {
  background-image: url(../../../assets/direction-icon.png);
  width: 20px;
  height: 20px;
  top: 15px;
}
...
export default{
  ...
  data(){
    ...
    methods:{
      UpdateDepartement(){
        const path = `http://127.0.0.1:5000/UpdateDepartement/${this.id}`
        axios.put(path, {
          name:this.name,
          nameDirection:this.nameDirection,
          })
        .then((res) => {
              if (res.data == "0" ){
      ...
      if (result.isConfirmed) {
        window.location.href = "http://localhost:8081/ListDepartement";
      }
      })
    },
    },created(){
        ...
        this.nameDirection = this.$route.params.nameDirection;
    }
}
</script>
